...
export async function deshuffle(buf: ArrayBuffer, { seed, blockWidth = DEFAULT_BLOCK_WIDTH }: DeshuffleOptions) {
    ...
    // need one pass here because we need the values generated by prng from "the back"
    for (let blockNum = totalBlocksNum - 1; blockNum > 0; blockNum = blockNum - 1) {
        permutations.push(rand(0, blockNum - 1, prng))
    }
    const permLen = permutations.length

    for (var blockNum = 1; blockNum < totalBlocksNum; blockNum = blockNum + 1) {
        const swapWith = permutations[permLen - blockNum]
